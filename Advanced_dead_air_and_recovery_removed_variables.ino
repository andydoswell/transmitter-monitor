...
#define lampPin 8
#define RXRebootPin 9
#define TXRebootPin 8
...
void loop() {
  ...
  if (shutDownFlag) { // this is set true if the shut down interrupt is, er , interrupted.
    ...
    for (int i = 0; i <= 240; i++); { // allow 2 minutes for the Pi's to safely shut down, and blink the VU lamps & error LED while they shut down, to give the user a clue.
      digitalWrite (lampPin, HIGH);// VU lamps off
...
boolean testRX() {
  for (int i = 0; i <= 30000; i++) { // get 30000 samples from our received audio, and average them
    unsigned long RXaudioValue = analogRead (A0);
    unsigned long RXAverage = RXAverage + RXaudioValue;
  }
  unsigned long RXAverage = RXAverage / 30000;
...
boolean testTX() {
  for (int i = 0; i <= 30000; i++) {// get 30000 samples from our incoming audio, and average them
    unsigned long TXaudioValue = analogRead (A3);
    unsigned long TXAverage = TXAverage + TXaudioValue;
  }
  unsigned long TXAverage = TXAverage / 30000;
...
void resetRX () {
  ...
  pinMode (RXRebootPin, INPUT);
  delay (180000); // delay for 3 mins to allow the pi to shutdown and restart
}
